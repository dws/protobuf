...
#define UPB_DECODEFAST_CARDINALITIES(_, ...) \
  _(__VA_ARGS__, Scalar)                     \
  _(__VA_ARGS__, Oneof)                      \
  _(__VA_ARGS__, Repeated)                   \
  _(__VA_ARGS__, Packed)

#define UPB_DECODEFAST_TYPES(_, ...) \
  _(__VA_ARGS__, Bool)               \
  _(__VA_ARGS__, Varint32)           \
  _(__VA_ARGS__, Varint64)           \
  _(__VA_ARGS__, ZigZag32)           \
  _(__VA_ARGS__, ZigZag64)           \
  _(__VA_ARGS__, Fixed32)            \
  _(__VA_ARGS__, Fixed64)            \
  _(__VA_ARGS__, ClosedEnum)         \
  _(__VA_ARGS__, String)             \
  _(__VA_ARGS__, Bytes)              \
  _(__VA_ARGS__, Message64)          \
  _(__VA_ARGS__, Message128)         \
  _(__VA_ARGS__, Message192)         \
  _(__VA_ARGS__, Message256)         \
  _(__VA_ARGS__, MessageBig)

#define UPB_DECODEFAST_TAGSIZES(_, ...) \
  _(__VA_ARGS__, Tag1Byte)              \
  _(__VA_ARGS__, Tag2Byte)
...
// There are only 14 impossible combinations out of 112 total, so it's not
// worth optimizing for.
#define UPB_DECODEFAST_FUNCTIONS(_) \
  UPB_DECODEFAST_TYPES(UPB_DECODEFAST_CARDINALITIES, UPB_DECODEFAST_TAGSIZES, _)
